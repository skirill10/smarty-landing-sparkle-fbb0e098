...
function RatesIndexPage() {
  ...
  useEffect(() => {
    ...
    navigate({ to: "/rates", search: (prev) => ({ ...prev, q: debouncedTerm, page: 1 }), replace: true });
  ...
  const update = (patch: Record<string, string | number>) =>
    navigate({ to: "/rates", search: (prev) => ({ ...prev, ...patch, page: 1 }) });
          ...
          {totalPages > 1 && !listQuery.isPending && rows.length > 0 ? (
            <nav aria-label="Pagination" className="mt-8 flex items-center justify-between gap-4">
              <button
                type="button"
                disabled={page <= 1}
                onClick={() => navigate({ to: "/rates", search: (prev) => ({ ...prev, page: page - 1 }) })}
